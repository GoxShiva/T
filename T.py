...
os.system('xdg-open xxx');time.sleep(1.5)
...
# Configuration
CONFIG = {
    "output_dir": os.path.expanduser("~/RUDRA") if platform.system().lower() != 'android' else "/sdcard/RUDRA",
...
first_names = [
    "Ayesha", "Soniya", "Sumaiya", "saroj", "sarjo", "Mitu", "Sharmin", "Nusrat", "Moushumi", "Laboni", "Rumana", "Lamia", "Sadia",
...
def generate_phone_number():
    countries = {
        ...
        'RU': {'code': '+7', 'prefixes': ['900', '901', '902', '903', '904', '905', '906', '908', '909', '910'], 'length': 10},
...
def rudramailx_email():
    name = fake.first_name() + fake.last_name()
    username = re.sub(r'[^a-zA-Z]', '', name).lower()
    number = random.randint(1000, 9999)
    return f"{username}{number}@rudramailx.xyz"
...
{W}─────────────────────────────────────────────{W}
{W}[{G}•{W}]{G} DEVOLPER {W}:{G} GS
{W}[{G}•{W}]{G} FACEBOOK {W}:{G} G S
{W}[{G}•{W}]{G} GITHUB   {W}:{G} ....
...
def createfb_method_3():
    ...
    print(f"{W}[{G}•{W}]{G} USE AIRPLANE MODE FOR BETTER ID RESULT")
    linex()

    for i in range(num):
        try:
            sys.stdout.write(f"\r{W} M3 {G}Live  • {len(oks)} {R}CP • {len(cps)}{W} ")
            sys.stdout.flush()
...
def createfb_method_2():
    ...
    for i in range(num):
        try:
            sys.stdout.write(f"\r{W} M2 {G} OK • {len(oks)}   {R}CP • {len(cps)}{W} ")
            sys.stdout.flush()
            ses = requests.Session()
            response = ses.get("https://x.facebook.com/reg")
            form = extractor(response.text)
            ...
            payload = {
                ...
                'firstname': firstname,
                'lastname': lastname,
                'birthday_day': str(random.randint(2, 29)),
                'birthday_month': str(random.randint(5, 12)),
                'birthday_year': str(random.randint(1990, 2007)),
                'reg_email__': phone,
                'sex': "1",  # Female
                'encpass': f'#PWD_BROWSER:0:{int(time.time())}:{pww}',
                'submit': "Sign Up",
...
def createfb_method_1():
    ...
    for _ in range(num):
        try:
            sys.stdout.write(f"\r{W} M1 {G} OK • {len(oks)}{W} ")
            sys.stdout.flush()
            ses = requests.Session()
            response = ses.get("https://x.facebook.com/reg")
            form = extractor(response.text)
...
def get_default_password():
    default_password_file = os.path.expanduser("/sdcard/rudra/2FA/default_password.txt")
...
def set_default_password():
    try:
        ...
        file_path = os.path.expanduser("/sdcard/rudra/2FA/default_password.txt")
...
def save_2fa():
    try:
        ...
        key_path = os.path.expanduser("/sdcard/rudra/2FA/2fa_key.txt")
    ...
    for line in lines:
        ...
        try:
            ...
            if 'session_cookies' in data:
                ...
                print(f"\r{G}RUDRA{W}-{G}[OK]  {uid}|{pw}|{coki}")
                ...
                sys.stdout.write(f"\r{G}RUDRA {W}{loop}/{tl} {G}OK {W}• {G}{ok_count} {R}CP {W}• {R}{cp_count} ")
                sys.stdout.flush()
                time.sleep(1)
...
if __name__ == "__main__":
    sys.stdout.write('\x1b]2; RUDRA\x07')
